-- Exported from production supabase_migrations.schema_migrations (20260919092201).
...
create or replace function public.accept_shared_record(p_notification_id uuid,p_to_user_id uuid) returns jsonb language sql security invoker set search_path='' as $$ select private.accept_shared_record(p_notification_id,p_to_user_id); $$;
create or replace function public.create_team_for_user(p_name text,p_user_id uuid) returns json language sql security invoker set search_path='' as $$ select private.create_team_for_user(p_name,p_user_id); $$;
create or replace function public.current_team_id() returns uuid language sql security invoker set search_path='' as $$ select private.current_team_id(); $$;
create or replace function public.get_team_member_emails(p_team_id uuid) returns table(user_id uuid,role text,joined_at timestamptz,email text) language sql security invoker set search_path='' as $$ select * from private.get_team_member_emails(p_team_id); $$;
create or replace function public.join_team_by_code(p_invite_code text,p_user_id uuid) returns json language sql security invoker set search_path='' as $$ select private.join_team_by_code(p_invite_code,p_user_id); $$;
create or replace function public.migrate_user_data_to_team(p_user_id uuid,p_team_id uuid) returns void language sql security invoker set search_path='' as $$ select private.migrate_user_data_to_team(p_user_id,p_team_id); $$;
create or replace function public.notify_assignment(p_to_user_id uuid,p_from_user_id uuid,p_team_id uuid,p_record_type text,p_record_id uuid,p_record_title text,p_message text) returns void language sql security invoker set search_path='' as $$ select private.notify_assignment(p_to_user_id,p_from_user_id,p_team_id,p_record_type,p_record_id,p_record_title,p_message); $$;
create or replace function public.reassign_record(p_table text,p_record_id uuid,p_assigned_to_user_id uuid,p_assigned_to text) returns void language sql security invoker set search_path='' as $$ select private.reassign_record(p_table,p_record_id,p_assigned_to_user_id,p_assigned_to); $$;
create or replace function public.same_team(record_team_id uuid) returns boolean language sql security invoker set search_path='' as $$ select private.same_team(record_team_id); $$;
