...
// UPDATED: Added Calendar + Team Dashboard + Jobs + Invoices entries.
...
export function NavDrawer({ open, onClose, currentScreen, onNavigate, badges = {}, userEmail, onLogout }) {
  ...
  const [workflowUserId, setWorkflowUserId] = useState(null);

  useEffect(() => {
    if (!workflowScreen) return;
    let active = true;
    supabase.auth.getUser().then(({ data }) => {
      if (active) setWorkflowUserId(data?.user?.id || null);
    }).catch(() => {});
    return () => { active = false; };
  }, [workflowScreen]);

  function go(key) {
    if (key === "Jobs" || key === "Invoices") {
      setWorkflowScreen(key);
      onClose();
      return;
    }
    onNavigate(key);
  ...
  return (
    <>
          ...
          <>
            ...
            <motion.div
              ...
              <div className="px-5 pt-12 pb-4 flex items-end justify-between" style={{ background: "linear-gradient(135deg, #8B1A1A 0%, #6B1414 100%)" }}>
                ...
                <button onClick={onClose} className="w-8 h-8 rounded-full flex items-center justify-center shrink-0 mb-0.5" style={{ background: "rgba(255,255,255,0.15)" }}>
                    ...
                    {section.items.map(item => {
                      ...
                      return (
                        <button key={item.key} onClick={() => go(item.key)} className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-colors min-h-[52px] mx-1 rounded-xl ${active ? "bg-red-50" : "hover:bg-slate-50"}`} style={{ width: "calc(100% - 8px)" }}>
                          <div className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0" style={{ background: active ? "#F7F3F3" : "#F8FAFC" }}>
      ...
      {workflowScreen && workflowUserId && (
        <div className="fixed inset-0 z-[80] overflow-y-auto bg-slate-50">
          <div className="sticky top-0 z-10 flex items-center justify-between bg-white/95 backdrop-blur border-b border-slate-200 px-4 py-3">
            <p className="font-black text-slate-900">{workflowScreen === "Jobs" ? "Jobs" : "Invoices"}</p>
            <button onClick={() => setWorkflowScreen(null)} className="rounded-xl border border-slate-200 px-3 py-2 text-sm font-bold text-slate-600">Close</button>
          </div>
          <div className="mx-auto max-w-2xl px-4 py-4">
            {workflowScreen === "Jobs"
              ? <JobsScreen userId={workflowUserId} />
              : <InvoicesScreen userId={workflowUserId} />}
          </div>
        </div>
      )}
    </>
