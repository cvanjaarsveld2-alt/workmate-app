      ...
      <AnimatePresence>
        {financeOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[80] bg-slate-100 overflow-y-auto"
          >
            <div className="min-h-screen px-4 pt-[calc(1rem+env(safe-area-inset-top))] pb-10">
              <div className="mx-auto max-w-2xl space-y-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-xl font-black text-slate-900">Jobs & Billing</p>
                    <p className="text-sm text-slate-500">Quote → job → invoice → payment</p>
                  </div>
                  <button
                    onClick={() => setFinanceOpen(false)}
                    className="w-11 h-11 rounded-xl bg-white border border-slate-200 flex items-center justify-center"
                    aria-label="Close Jobs & Billing">
                    <X size={20} />
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    onClick={() => setFinanceTab("jobs")}
                    className={`rounded-xl py-3 text-sm font-black ${financeTab === "jobs" ? "bg-slate-900 text-white" : "bg-white text-slate-600 border border-slate-200"}`}>
                    Jobs
                  </button>
                  <button
                    onClick={() => setFinanceTab("invoices")}
                    className={`rounded-xl py-3 text-sm font-black ${financeTab === "invoices" ? "bg-slate-900 text-white" : "bg-white text-slate-600 border border-slate-200"}`}>
                    Invoices & Payments
                  </button>
                </div>

                {financeTab === "jobs"
                  ? <JobsScreen userId={onNavigate?.userId} teamId={null} />
                  : <InvoicesScreen userId={onNavigate?.userId} teamId={null} />}
              </div>
            </div>
          </motion.div>
        )}
