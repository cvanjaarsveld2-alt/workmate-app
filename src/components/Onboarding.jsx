...
export function Onboarding({ userId, onDone }) {
  ...
  function next() { haptic.light(); last ? finish() : setI(i + 1); }
  function finish() {
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-8 text-center">
        <AnimatePresence mode="wait">
          <motion.div key={i}
            ...
            <div className="w-24 h-24 rounded-3xl flex items-center justify-center mb-8" style={{ background: `${slide.color}18` }}>
              <Icon size={44} style={{ color: slide.color }} />
            </div>
            <h1 className="text-2xl font-black text-slate-900 mb-3 leading-tight">{slide.title}</h1>
            <p className="text-base text-slate-500 leading-relaxed max-w-sm">{slide.body}</p>
          </motion.div>
        </AnimatePresence>
      </div>
        ...
        <div className="flex justify-center gap-2">
          {SLIDES.map((_, idx) => (
            <div key={idx} className="h-2 rounded-full transition-all duration-300"
              style={{ width: idx === i ? 24 : 8, background: idx === i ? slide.color : "#E2E8F0" }} />
          ))}
        </div>
        <button onClick={next}
...
export function shouldShowOnboarding(userId) {
  try { return localStorage.getItem(`pm_onboarded_${userId}`) !== "1"; } catch { return false; }
}
