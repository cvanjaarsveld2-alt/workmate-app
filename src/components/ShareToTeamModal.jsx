// ─── Share To Team Modal ──────────────────────────────────────────────────────
// Drop this on any screen that has a record (Client, Contact, Lead, Follow-up).
// It handles the full send → accept/decline notification loop.
//
// Usage:
...
//     record={shareTarget}          // { id, title, type }
//     fromUserId={userId}
//     fromEmail={userEmail}
//     teamId={teamId}
...
//   />
//
// Add a share button to any card:
//   <button onClick={() => setShareTarget({ id: client.id, title: client.company, type: "client" })}>
//     <Send size={15} />
//   </button>
// ─────────────────────────────────────────────────────────────────────────────
...
function Avatar({ email, role, size = "md" }) {
  const letters = (email || "?").slice(0, 2).toUpperCase();
  const bg = role === "admin" ? "#A16207" : BRAND.primary;
  const dim = size === "sm" ? "w-8 h-8 text-xs" : "w-10 h-10 text-sm";
  return (
    <div className={`${dim} rounded-full flex items-center justify-center text-white font-black shrink-0`}
      style={{ background: bg }}>
      {letters}
    </div>
  );
...
export function ShareToTeamModal({
  ...
  async function handleSend() {
    ...
    const fullMessage = message.trim()
      ? `${fromName} shared a ${typeLabel} with you: ${record.title} — "${message.trim()}"`
      : `${fromName} shared a ${typeLabel} with you: ${record.title}`;

    try {
      // 1. Write the in-app notification (uses security-definer RPC so RLS doesn't block)
      ...
      // 2. Push notification (best-effort — don't block on failure)
      try {
        ...
        for (const sub of subs || []) {
          await supabase.functions.invoke("send-notifications", {
            body: {
              ...
              title: `PowerMate — ${typeMeta.label} shared with you`,
              body:  fullMessage,
              url:   "/?screen=SharedInbox",
            },
          }).catch(() => {}); // silence individual push failures
        }
      } catch {}
          ...
          <motion.div
            ...
            <div className="flex items-center justify-between px-5 pb-4 border-b border-slate-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-base font-black text-slate-900 leading-tight">Share with teammate</p>
              ...
              {status === "sent" ? (
                <motion.div
                  ...
                  <div className="text-center">
                    <p className="text-lg font-black text-slate-900">Sent!</p>
                    <p className="text-sm text-slate-500 mt-1">
                      {recipients.find(m => m.user_id === selectedId)?.email?.split("@")[0] || "Your teammate"} will
                      see it in their Shared Inbox.
                    </p>
                ...
                <div className="px-5 py-4 space-y-5">
                  ...
                  {recipients.length === 0 ? (
                    <div className="py-8 text-center">
                      ...
                      <p className="text-xs text-slate-400 mt-1">Invite teammates from Team & Sharing</p>
                    </div>
                  ) : (
                    <>
                      <div>
