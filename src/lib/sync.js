...
export async function pullFromSupabase(uid, setData) {
  try {
    ...
    const [a, b, c, d, leads_res, e, f, g, h, act, bd, rep, cf] = await Promise.all([
      supabase.from("clients").select("id,user_id,team_id,company,division,contact,phone,email,location,branch,stage,category,sync_status,auto_created,source,notes,assigned_to_user_id,assigned_to,created_at,updated_at").order("created_at", { ascending: false }).limit(1000),
