...
export async function pullFromSupabase(uid, setData) {
  try {
    ...
    const [a, b, c, d, leads_res, e, f, g, h, act] = await Promise.all([
      ...
      supabase.from("contacts").select("id,user_id,team_id,name,company,title,email,phone,met_at,met_date,notes,card_photo_url,status,client_id,assigned_to_user_id,assigned_to,sync_status,created_at,updated_at").order("created_at", { ascending: false }).limit(1000),
      ...
      supabase.from("notes").select("id,user_id,team_id,client,client_id,note,urgency,resolve_by,resolved,resolved_at,last_escalated,media,linked_contact_ids,assigned_to_user_id,assigned_to,sync_status,created_at").order("created_at", { ascending: false }).limit(1000),
