...
const REMOTE_EXCLUDED_FIELDS = {
  quotes: new Set([
    ...
    "sync_pending_job_id",
    "sync_pending_client_id",
    "sync_pending_note_id",
    "sync_pending_team_id",
...
const DEPENDENCIES = {
  followups: [
    { field: "quote_id", pending: "sync_pending_quote_id", table: "quotes" },
    { field: "client_id", pending: "sync_pending_client_id", table: "clients" },
    { field: "linked_note_id", pending: "sync_pending_note_id", table: "notes" },
    { field: "team_id", pending: "sync_pending_team_id", table: "teams" },
  ],
  jobs: [
    { field: "quote_id", pending: "sync_pending_quote_id", table: "quotes" },
    { field: "client_id", pending: "sync_pending_client_id", table: "clients" },
  ...
  invoices: [
    { field: "quote_id", pending: "sync_pending_quote_id", table: "quotes" },
    { field: "job_id", pending: "sync_pending_job_id", table: "jobs" },
    { field: "client_id", pending: "sync_pending_client_id", table: "clients" },
  ],
...
const SYNC_PRIORITY = {
  ...
  vehicle_checks: 30,
  activities: 30,
  breakdown_reports: 30,
  repair_reports: 30,
  custom_faults: 30,
  service_reports: 30,
  team_notifications: 30,
  email_quotes: 35,
  followups: 40,
  jobs: 50,
  invoices: 60,
...
const ARRAY_CONFLICT_FIELDS = {
  jobs: ["photos", "parts_used"],
  breakdown_reports: ["items"],
  repair_reports: ["items"],
};
...
function serverChangedSinceBase(existing, incoming) {
  const serverTs = Date.parse(existing?.updated_at || ""),
    baseTs = Date.parse(incoming?.updated_at || "");
  ...
  for (const field of fields) {
    ...
    if (arraysDiffer(existing[field], incoming[field])) {
      const error = new Error(
        "This " +
          table.replaceAll("_", " ") +
          " was changed on another device while this device was offline. The newer server array was not overwritten.",
      );
...
function normalizeVehicleCheckPayload(data) {
  const source = data || {};
  const out = {
    id: source.id,
  ...
  if (!out.id) throw new Error("vehicle_checks record is missing id");
  if (!out.check_date || !/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(String(out.check_date)))
    throw new Error("vehicle_checks record has an invalid check_date");
  return out;
}
...
function stripEmbeddedBase64(value, depth = 0) {
  ...
  if (typeof value === "object") {
    ...
    for (const k of Object.keys(value)) {
      ...
      out[k] = stripEmbeddedBase64(value[k], depth + 1);
    }
    return out;
  }
  return value;
...
function regenerateNumber(prefix) {
  const year = new Date().getFullYear();
  return `${prefix}-${year}-${String(Date.now()).slice(-6)}${Math.floor(Math.random() * 10)}`;
}
...
async function stageMissingDependencies(table, payload) {
  const deps = DEPENDENCIES[table] || [];
  if (!deps.length) return payload;
  let out = { ...payload };
  for (const dep of deps) {
    const id = out[dep.field];
    if (!id || out[dep.pending]) continue;
    const { data, error } = await supabase.from(dep.table).select("id").eq("id", id).maybeSingle();
...
async function pushOne(table, action, rawData) {
  ...
  if (["insert", "upsert", "update"].includes(action)) {
    if (table === "vehicle_checks") {
      const { data: authData, error: authError } = await supabase.auth.getUser();
      if (authError) throw authError;
      if (authData?.user?.id) payload.user_id = authData.user.id;
      else throw new Error("Cannot sync vehicle check without an authenticated user");
    } else if (TEAM_TABLES.has(table) && !payload.user_id) {
    ...
    if (action === "update" && payload.id) {
      ...
      if (existing) {
        assertNoStaleArrayOverwrite(table, existing, payload);
...
export async function pushItem(item) {
  try {
    ...
    const result = await pushOne(table, item.action, item.data);
    return { ok: true, duplicate: !!result?.duplicate, canonical: result?.canonical || null };
  } catch (error) {
    const detail = {
      code: error?.code,
    ...
    detail.syncErrorCode = classification.code;
    console.warn(`[Sync] FAILED ${item?.table} ${item?.action}`, detail);
    if (classification.code !== "NETWORK_ERROR") {
      try {
        logCrash({
          screen: `Sync (${item?.table} ${item?.action})`,
          message: `[${classification.code}] ${detail.message}${detail.code ? ` (${detail.code})` : ""}${detail.details ? ` — ${detail.details}` : ""}`,
        });
      } catch {}
    }
    return {
      ok: false,
      error: detail,
      networkError: classification.code === "NETWORK_ERROR",
      classification,
    };
  }
}
async function persistQueue(queue) {
  await offlineReplaceAll("syncQueue", queue);
}
...
function applyLocalRecord(current, local, item) {
  ...
  const next = index >= 0 ? rows.map((row, i) => (i === index ? item : row)) : [item, ...rows];
...
export async function saveAndSync(item, table, action, setData, isOnline) {
  ...
  const canonical =
    result.duplicate && result.canonical ? result.canonical : { ...item, sync_status: "synced" };
  ...
  await offlineReplaceAll(
    "syncQueue",
    durableQueue.filter(q => !settled(q)),
  );
  if (newerQueued) {
...
function collapseQueue(queue) {
  ...
  for (const [, ops] of groups) {
    ...
    const last = ops[ops.length - 1],
      hasInsert = ops.some(op => op.action === "insert"),
      hasDelete = ops.some(op => op.action === "delete");
    if (hasInsert && hasDelete) {
      ops.forEach(op => discarded.add(op.id));
      continue;
    }
    if (hasDelete) winners.push({ ...last, action: "delete" });
      ...
      winners.push({
        ...last,
        action: hasInsert ? "upsert" : "update",
        data: ops.reduce((record, op) => ({ ...record, ...(op.data || {}) }), {}),
      });
    const winner = winners[winners.length - 1];
    ops.forEach(op => {
      if (op.id !== winner.id) discarded.add(op.id);
    });
  }
...
function sortSyncItems(items) {
  return [...items].sort((a, b) => {
    const pa = SYNC_PRIORITY[a.table] ?? 100,
      pb = SYNC_PRIORITY[b.table] ?? 100;
    if (pa !== pb) return pa - pb;
    return new Date(a.created_at || 0) - new Date(b.created_at || 0);
  });
}
function dependencyReady(item, queue) {
  const deps = DEPENDENCIES[item.table] || [];
  for (const dep of deps) {
    const id = item.data?.[dep.field];
    if (!id) continue;
    const parentQueued = queue.some(
      q => q.status === "pending" && q.table === dep.table && q.data?.id === id && q.action !== "delete",
    );
    if (parentQueued) return false;
  }
  return true;
}
function backoffMs(attempts) {
  return Math.min(60000, 1000 * Math.pow(2, Math.max(0, attempts - 1)) + Math.floor(Math.random() * 750));
}
function newQueueId() {
...
async function reattachStagedDependencies(setData) {
  ...
  try {
    for (const table of Object.keys(DEPENDENCIES)) {
      ...
      let rows;
      try {
        rows = await offlineGetAll(local);
      } catch {
        continue;
      }
      for (const row of rows) {
        ...
        for (const dep of deps) {
          ...
          try {
            const { data, error } = await supabase
              ...
              .maybeSingle();
            if (!error && data) {
              patch[dep.field] = pendingVal;
              patch[dep.pending] = null;
              changed = true;
            }
          } catch {
            /* transient — will be retried on the next successful sync pass */
          }
        }
        if (!changed) continue;
        ...
        setData(current => ({
          ...current,
          [local]: (current[local] || []).map(r => (r.id === updated.id ? updated : r)),
          syncQueue: [
            queueItem,
            ...(current.syncQueue || []).filter(q => !(q.table === table && q.data?.id === updated.id)),
          ],
        }));
        await offlineSave("syncQueue", queueItem);
      }
    ...
    console.warn("[Sync] reattachStagedDependencies failed", e);
  }
}
export async function pushSyncQueue(syncQueue, setData) {
  ...
  try {
    ...
    const revived = syncQueue.map(item =>
      item.status === "failed" &&
      !item.revived_v2 &&
      REVIVABLE_CODES.has(item.last_error?.code || item.last_error?.syncErrorCode)
        ? { ...item, status: "pending", attempts: 0, next_attempt_at: null, revived_v2: true }
        : item,
    );
    if (revived.some((item, i) => item !== syncQueue[i])) {
      syncQueue = revived;
      await persistQueue(syncQueue);
    }
  } catch (e) {
    console.warn("[Sync] durable queue read failed; using in-memory queue", e);
  }
  ...
  try {
    ...
    for (const item of ordered) {
      ...
      if (!dependencyReady(item, queueSnapshot)) {
        outcomes.push({
          queueId: item.id,
          ...
          ok: false,
          blocked: true,
          error: { code: "SYNC_DEPENDENCY_WAIT", message: `Waiting for ${item.table} dependencies` },
        });
        continue;
      }
    ...
    const succeeded = outcomes.filter(r => r.ok),
      failed = outcomes.filter(r => !r.ok && !r.blocked),
      blocked = outcomes.filter(r => r.blocked);
    for (const result of succeeded)
      if (result.action === "delete")
        offlineDelete(localStoreName(result.table), result.entityId).catch(() => {});
    ...
    if (failed.length) {
      ...
      window.dispatchEvent(
        new CustomEvent("powermate:sync_failed", {
          detail: {
            count: failed.length,
            message: `${failed.length} item${failed.length === 1 ? "" : "s"} failed to sync`,
            items: failed.map(f => ({
              table: f.table,
              entityId: f.entityId,
              syncErrorCode: f.error?.syncErrorCode || null,
            })),
          },
        }),
      );
    ...
    const succeededIds = new Set(succeeded.map(r => r.queueId)),
      failedIds = new Set(failed.map(r => r.queueId));
    ...
    const nextQueue = (queueSnapshot || [])
      .filter(item => !succeededIds.has(item.id) && !discarded.has(item.id))
      .map(original => {
        const winner = winnerById.get(original.id);
        const item =
          winner && !succeededIds.has(original.id)
            ? { ...original, action: winner.action, data: winner.data }
            : original;
        if (!failedIds.has(item.id)) return item;
        const outcome = failed.find(r => r.queueId === item.id);
        const classification =
          outcome?.classification ||
          classifySyncError({ code: outcome?.error?.code, message: outcome?.error?.message });
        ...
        // resolve itself.
        if (!networkError && (!classification.retryable || attempts >= MAX_SYNC_ATTEMPTS))
          return { ...item, attempts, status: "failed", last_error: outcome?.error || null };
        return {
          ...item,
          attempts,
          status: "pending",
          next_attempt_at: new Date(Date.now() + backoffMs(Math.max(1, attempts))).toISOString(),
    ...
    setData(current => {
      ...
      for (const result of succeeded) {
        const key = localStoreName(result.table);
        if (result.action === "delete")
          next[key] = (next[key] || []).filter(row => row.id !== result.entityId);
          ...
          next[key] = [
            { ...result.canonical, sync_status: "synced" },
            ...rows.filter(row => row.id !== result.canonical.id),
          ];
        } else if (result.entityId)
          next[key] = (next[key] || []).map(row =>
            row.id === result.entityId ? { ...row, sync_status: "synced" } : row,
          );
      }
      return next;
    });
    ...
    if (blocked.length)
      setTimeout(() => {
        if (_globalSetData && _globalQueueRef)
          pushSyncQueue(_globalQueueRef.current || [], _globalSetData).catch(() => {});
      }, 1500);
  } finally {
    ...
    if (_syncRerunRequested) {
      _syncRerunRequested = false;
      if (_globalSetData && _globalQueueRef)
        setTimeout(() => pushSyncQueue(_globalQueueRef.current || [], _globalSetData).catch(() => {}), 50);
    }
  }
}
...
function dirtyQueueForTable(table) {
  return (_globalQueueRef?.current || []).filter(
    q => (q.status === "pending" || q.status === "failed") && q.table === table,
  );
}
  ...
  try {
    ...
    for (let i = 0; i < SYNC_TABLES.length; i++) {
      const table = SYNC_TABLES[i],
        result = results[i];
      if (result.error) {
        console.warn(`[Sync] pull failed: ${table}`, result.error);
        continue;
      }
      const local = localStoreName(table);
      const dirty = (durableQueue || []).filter(
        q => (q.status === "pending" || q.status === "failed") && q.table === table,
      );
...
async function retryFlatMedia(table, setData) {
  ...
  for (const row of rows) {
    ...
    const pending = media
      .map((m, i) => [m, i])
      .filter(([m]) => m && !m.url && (m.base64 || m.file) && m.uploadStatus !== "done");
    if (!pending.length) continue;
    const nextMedia = [...media];
    let changed = false;
    for (const [m, i] of pending) {
      try {
        const uploaded = await uploadPhotoToSupabaseWithPath(
          m.file || m.base64,
          `${table}/${row.id}/${m.id}`,
        );
        if (uploaded) {
        ...
        console.warn(`[Sync] media retry failed for ${table}/${row.id}/${m.id}`, e);
      }
    }
    if (!changed) continue;
    const updated = { ...row, media: nextMedia, sync_status: "pending" };
    await offlineSave(local, updated);
    await queueMediaCorrection(
      table,
      { ...updated, media: updated.media.map(x => ({ ...x, base64: undefined })) },
      setData,
    );
  }
  return any;
      ...
      for (const p of photos) {
        if (p && typeof p.url === "string" && p.url.startsWith("data:")) {
          try {
            const uploaded = await uploadPhotoToSupabaseWithPath(
              p.url,
              `${local}/${uid}/${row.id}/${p.id}.jpg`,
            );
            if (uploaded) {
              nextPhotos.push({ ...p, url: uploaded.url, storage_path: uploaded.path });
              changed = true;
              any = true;
            } else nextPhotos.push(p);
          } catch (e) {
            console.warn(`[Sync] report media retry failed for ${table}/${row.id}/${p.id}`, e);
            nextPhotos.push(p);
          }
        } else nextPhotos.push(p);
      }
  ...
  for (const row of rows) {
    ...
    for (const [item, value] of Object.entries(nextItems)) {
      if (typeof value?.photo === "string" && value.photo.startsWith("data:")) {
        try {
          const uploaded = await uploadPhotoToSupabaseWithPath(
            value.photo,
            `vehicle-checks/${uid}/${row.check_date}/item-${item.replace(/[^a-z0-9]+/gi, "-").toLowerCase()}-${genId()}.jpg`,
          );
          if (uploaded) {
            nextItems[item] = { ...value, photo: uploaded.url, storage_path: uploaded.path };
            changed = true;
            any = true;
          }
        } catch (e) {
          console.warn("[Sync] vehicle item photo retry failed", e);
        }
      }
    }
    ...
    for (let i = 0; i < nextPhotos.length; i++) {
      ...
      try {
        const uploaded = await uploadPhotoToSupabaseWithPath(
          source,
          `vehicle-checks/${uid}/${row.check_date}/photo-${p.id || genId()}.jpg`,
        );
        if (uploaded) {
          nextPhotos[i] = { ...p, url: uploaded.url, storage_path: uploaded.path, _base64: null };
          changed = true;
          any = true;
        }
      } catch (e) {
        console.warn("[Sync] vehicle photo retry failed", e);
      }
    }
    if (!changed) continue;
    const updated = {
      ...row,
      data: { ...data, items: nextItems, photos: nextPhotos },
      sync_status: "pending",
    };
    await offlineSave(local, updated);
...
export async function retryPendingMedia(uid, setData) {
  ...
  try {
    ...
    console.warn("[Sync] retryPendingMedia failed", e);
    return false;
  }
}
      ...
      channel = channel.on("postgres_changes", { event: "*", schema: "public", table }, payload => {
        ...
        if (TEAM_TABLES.has(table)) {
          ...
          if (
            currentTeamId
              ...
              : !(row?.user_id === uid || row?.assigned_to_user_id === uid)
          )
            return;
        }
        ...
        setData(current => {
          ...
          if (payload.eventType === "DELETE")
            return { ...current, [local]: rows.filter(r => r.id !== oldRow?.id) };
          if (!row?.id) return current;
          const idx = rows.findIndex(r => r.id === row.id);
          return {
            ...current,
            [local]: idx >= 0 ? rows.map((r, i) => (i === idx ? row : r)) : [row, ...rows],
          };
        });
      });
      channel.subscribe();
