...
export async function pushSyncQueue(syncQueue,setData){if(_syncInProgress)return;_syncInProgress=true;try{const pending=(syncQueue||[]).filter(item=>item.status==="pending");if(!pending.length)return;const {winners,discarded}=collapseQueue(pending);const results=await Promise.allSettled(winners.map(async item=>({queueId:item.id,entityId:item.data?.id,table:item.table,action:item.action,...(await pushItem(item))})));const outcomes=results.map(r=>r.status==="fulfilled"?r.value:{ok:false,error:{message:"Unexpected sync failure"}});const succeeded=outcomes.filter(r=>r.ok),failed=outcomes.filter(r=>!r.ok);for(const result of succeeded)if(result.action==="delete")offlineDelete(localStoreName(result.table),result.entityId).catch(()=>{});if(failed.length){logEvent("sync_failed",{count:failed.length});window.dispatchEvent(new CustomEvent("powermate:sync_failed",{detail:{count:failed.length,message:`${failed.length} item${failed.length===1?"":"s"} failed to sync`}}));}if(succeeded.length)logEvent("sync_succeeded",{count:succeeded.length});const succeededIds=new Set(succeeded.map(r=>r.queueId));const failedIds=new Set(failed.map(r=>r.queueId));const succeededKeys=new Set(succeeded.map(r=>`${r.table}:${r.entityId}`));setData(current=>{const nextQueue=(current.syncQueue||[]).filter(item=>!succeededIds.has(item.id)&&!(discarded.has(item.id)&&succeededKeys.has(`${item.table}:${item.data?.id}`))).map(item=>{if(!failedIds.has(item.id))return item;const attempts=(item.attempts||0)+1;return attempts>=MAX_SYNC_ATTEMPTS?{...item,attempts,status:"failed"}:{...item,attempts,status:"pending"};});const next={...current,syncQueue:nextQueue};for(const result of succeeded){const key=localStoreName(result.table);if(result.action==="delete")next[key]=(next[key]||[]).filter(row=>row.id!==result.entityId);else if(result.entityId)next[key]=(next[key]||[]).map(row=>row.id===result.entityId?{...row,sync_status:"synced"}:row);}return next;});}finally{_syncInProgress=false;}}
