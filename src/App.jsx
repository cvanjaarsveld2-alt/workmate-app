...
function EquipmentScreen({ data, setData, userId }) {
  ...
  async function saveEquipment(){
    ...
    if(editId){
      setData(d=>({...d,equipment:(d.equipment||[]).map(e=>e.id===editId?{...e,...form,sync_status:"pending"}:e)}));
      await offlineSave("equipment",{...equipment.find(e=>e.id===editId),...form,sync_status:"pending"});
    } else {
      const item={id:genId(),user_id:userId,...form,created_at:new Date().toISOString(),sync_status:"pending"};
  ...
  function startEdit(e){setForm({name:e.name||"",type:e.type||"",make:e.make||"",model:e.model||"",serial:e.serial||"",location:e.location||"",client:e.client||"",service_due:e.service_due||"",notes:e.notes||""});setEditId(e.id);setShowForm(true);}
