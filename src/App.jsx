    ...
    if (this.state.hasError) return (
      <div className="flex min-h-screen flex-col items-center justify-center px-6 text-center" style={{ background: "var(--pm-page-bg)" }}>
        <div className="bg-white rounded-2xl max-w-sm w-full p-8 space-y-4 shadow-sm border border-slate-100">
          <div className="text-4xl">⚠️</div><h2 className="text-lg font-black text-slate-900">Something went wrong</h2>
          <button onClick={() => window.location.reload()} className="w-full rounded-2xl py-4 text-sm font-bold text-white min-h-[52px]" style={{ background: "#8B1A1A" }}>Reload App</button>
        </div>
      </div>
    );
...
export default function PowerWorksApp() {
  ...
  const [screen, setScreen] = useState(() => {
    try {
      ...
      const valid = ["Home","Clients","Contacts","Followups","Notes","Equipment","Quotes","Expenses","More","Diagnostics","BackfillZAR","Analytics","Leads","Team","VehicleCheck","Notifications","SharedInbox","Jobs","Invoices","Client360","Calendar","TeamDashboard"];
  ...
  useEffect(() => { if (!("serviceWorker" in navigator)) return; const onMessage = event => { if (event.data?.type === "POWERMATE_RETRY_SYNC") pushSyncQueue(syncQueueRef.current, setData); }; navigator.serviceWorker.addEventListener("message", onMessage); return () => navigator.serviceWorker.removeEventListener("message", onMessage); }, []);
  useEffect(() => {
    let mounted = true, settled = false;
    const finish = s => { if (mounted && !settled) { settled = true; setSession(s || null); setLoading(false); } };
    supabase.auth.getSession().then(({ data: { session: s } }) => finish(s));
    const failsafe = setTimeout(() => {
      if (settled) return; let cached = null;
      try { for (let i = 0; i < localStorage.length; i++) { const k = localStorage.key(i); if (k && k.endsWith("-auth-token")) { const v = JSON.parse(localStorage.getItem(k) || "null"); if (v && (v.access_token || v.currentSession || v.user)) cached = v.currentSession || v; break; } } } catch {}
      finish(cached);
    }, 1500);
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_, s) => { if (mounted) { setSession(s); setLoading(false); settled = true; } });
  ...
  useEffect(() => {
    if (!session?.user?.id) return; let pollInterval;
    async function loadTeamState() {
      try {
        const { data: membership } = await supabase.from("team_members").select("team_id, role").eq("user_id", session.user.id).maybeSingle();
        if (!membership?.team_id) return; setTeamId(membership.team_id); setUserRole(membership.role === "admin" || membership.role === "owner" ? "admin" : "member");
        const { data: rows, error: rpcError } = await supabase.rpc("get_team_member_emails", { p_team_id: membership.team_id });
        if (!rpcError && rows) setTeamMembers(rows); else { const { data: basicRows } = await supabase.from("team_members").select("user_id, role, joined_at").eq("team_id", membership.team_id); if (basicRows) setTeamMembers(basicRows.map(r => ({ ...r, email: r.user_id === session.user.id ? session.user.email : `Member ${r.user_id.slice(0, 8)}` }))); }
        async function checkUnread() { try { const { count } = await supabase.from("team_notifications").select("id", { count: "exact", head: true }).eq("to_user_id", session.user.id).eq("read", false); setUnreadCount(count || 0); } catch {} }
        checkUnread(); pollInterval = setInterval(checkUnread, 30000);
      } catch (e) { console.warn("Team load failed:", e); }
    }
    loadTeamState(); return () => clearInterval(pollInterval);
  }, [session?.user?.id]);
  ...
  useEffect(() => {
    if (!session?.user?.id) return; const uid = session.user.id; setOfflineUser(uid);
    async function loadLocalData() {
      try { const saved = localStorage.getItem(localStorageKey(uid)); if (saved) setData(d => ({ ...d, ...JSON.parse(saved) })); } catch (e) { console.warn("localStorage load failed:", e); }
      try {
        ...
        setData(d => ({ ...d, ...(clients?.length ? { clients } : {}), ...(followups?.length ? { followups } : {}), ...(quotes?.length ? { quotes } : {}), ...(notes?.length ? { notes } : {}), ...(equipment?.length ? { equipment } : {}), ...(contacts?.length ? { contacts } : {}), ...(expenses?.length ? { expenses } : {}), ...(leads?.length ? { leads } : {}), ...(activities ? { activities } : {}), ...(breakdowns ? { breakdowns } : {}), ...(repairs ? { repairs } : {}), ...(customFaults ? { customFaults } : {}), ...(syncQueue ? { syncQueue } : {}) }));
      } catch (e) { console.warn("IndexedDB load failed:", e); }
    }
    loadLocalData();
  }, [session?.user?.id]);
  useEffect(() => {
    const t = setTimeout(() => { try { const safeData = { ...data, notes: (data.notes || []).map(n => ({ ...n, media: (n.media || []).map(m => m.url ? { ...m, base64: undefined } : m) })), equipment: (data.equipment || []).map(e => ({ ...e, media: (e.media || []).map(m => m.url ? { ...m, base64: undefined } : m) })), syncQueue: (data.syncQueue || []).map(q => { if (!q.data?.media) return q; return { ...q, data: { ...q.data, media: q.data.media.map(m => ({ ...m, base64: undefined })) } }; }) }; const serialized = JSON.stringify(safeData); const sizeMB = (serialized.length / 1024 / 1024).toFixed(1); if (parseFloat(sizeMB) > 4) logEvent("localStorage_size_warning", { sizeMB }); localStorage.setItem(localStorageKey(session?.user?.id), serialized); } catch (e) { console.warn("Could not save local data:", e); if (e.name === "QuotaExceededError") logEvent("localStorage_quota_exceeded", { message: e.message }); } }, 500); return () => clearTimeout(t); }, [data]);
  useEffect(() => { if (!dataLoading && (data.quotes || []).length > 0) runQuoteAutomations(data, setData, session?.user?.id, teamId); }, [dataLoading]);
  ...
  useEffect(() => { if (!session) return; registerReminderPeriodicSync().catch(() => {}); }, [session]);
  useEffect(() => { if (!isOnline || !session) return; navigator.serviceWorker?.ready.then(reg => { if (reg.sync && typeof reg.sync.register === "function") reg.sync.register("powermate-sync").catch(() => {}); }).catch(() => {}); const t = setTimeout(() => pushSyncQueue(syncQueueRef.current, setData), 3000); return () => clearTimeout(t); }, [isOnline, session, data.syncQueue?.length]);
  const prevOnlineRef = useRef(isOnline);
  useEffect(() => { const wasOffline = !prevOnlineRef.current; prevOnlineRef.current = isOnline; if (isOnline && wasOffline && session) { pullFromSupabase(session.user.id, setData); if ((data.syncQueue || []).some(i => i.status === "pending")) pushSyncQueue(syncQueueRef.current, setData); processMediaQueue(setData).catch(() => {}); } }, [isOnline]);
  ...
  function navigate(key, ctx = {}) { if (key === screen && Object.keys(ctx).length === 0) return; setSearchSeed(null); setScreenContext(ctx); window.history.pushState({ pmScreen: key, pmContext: ctx }, ""); setScreen(key); }
