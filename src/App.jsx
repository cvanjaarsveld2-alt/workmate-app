...
export default function PowerWorksApp() {
  ...
  useEffect(() => {
    if (dataLoading) return; const today = todayISO(); const uid = session?.user?.id; const notes = (data.notes || []).filter(n => n.user_id === uid); const toEscalate = notes.filter(n => !n.resolved && n.resolve_by && n.resolve_by < today && n.last_escalated !== `${n.urgency}_${today}` && URGENCY_ESCALATION[n.urgency || "Normal"] !== (n.urgency || "Normal")); if (!toEscalate.length) return; const now = new Date().toISOString(); const updates = toEscalate.map(n => ({ ...n, urgency: URGENCY_ESCALATION[n.urgency || "Normal"], last_escalated: `${URGENCY_ESCALATION[n.urgency || "Normal"]}_${today}`, sync_status: "pending" })); const queueItems = updates.map(u => ({ id: genId(), table: "notes", action: "update", data: { ...u, media: (u.media || []).map(m => ({ ...m, base64: undefined })) }, status: "pending", created_at: now })); setData(d => ({ ...d, notes: (d.notes || []).map(n => updates.find(u => u.id === n.id) || n), syncQueue: [...queueItems, ...(d.syncQueue || [])] })); updates.forEach(u => offlineSave("notes", u)); triggerImmediateSync();
  }, [dataLoading]);
  ...
  async function clearAuthAndSignOut() {
    ...
    if (unsyncedItems.length > 0) { const pending = unsyncedItems.filter(i => i.status === "pending").length; const failed = unsyncedItems.filter(i => i.status === "failed").length; const parts = []; if (pending > 0) parts.push(`${pending} pending`); if (failed > 0) parts.push(`${failed} failed`); if (!window.confirm(`You have ${parts.join(" and ")} unsynced change${unsyncedItems.length !== 1 ? "s" : ""}. Signing out will delete them.\n\nSign out anyway?`)) return; }
    localStorage.removeItem(PIN_KEY); sessionStorage.removeItem(PIN_UNLOCKED_KEY); localStorage.removeItem("pm_pin_disabled"); resetPINAttempts(); await clearAllStores(); if (session?.user?.id) localStorage.removeItem(localStorageKey(session.user.id));
    setData({ clients: [], followups: [], quotes: [], notes: [], equipment: [], contacts: [], expenses: [], leads: [], activities: [], breakdowns: [], repairs: [], customFaults: [], syncQueue: [] }); setTeamId(null); setTeamMembers([]); setUserRole("member"); setUnreadCount(0); setScreenContext({}); setDataLoading(true); try { await supabase.auth.signOut(); } catch (e) { console.warn("Sign out failed:", e); } setSession(null);
  }
      ...
      <ExportProgressProvider>
        <div className="min-h-screen pb-32" style={{ background: "var(--pm-page-bg)", paddingTop: "calc(3.5rem + env(safe-area-inset-top))" }}>
          ...
          <main className="mx-auto max-w-2xl px-4 pt-4">
            <PullToRefresh onRefresh={async () => { if (!isOnline || !session?.user?.id) return; await pushSyncQueue(data, setData); await pullFromSupabase(session.user.id, setData); }}>
              <AnimatePresence mode="wait"><motion.div key={screen} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.15 }}><ScreenErrorBoundary label={screen} onGoHome={() => navigate("Home")}><Suspense fallback={<div className="flex justify-center items-center py-20"><Spinner /></div>}>{screens[screen]}</Suspense></ScreenErrorBoundary></motion.div></AnimatePresence>
            </PullToRefresh>
          </main>
          ...
          <AnimatePresence>{syncError && <Toast message={syncError} type="error" onDone={() => setSyncError("")} />}{showOnboarding && session?.user?.id && <Onboarding userId={session.user.id} onDone={() => setShowOnboarding(false)} />}</AnimatePresence>
        </div>
      </ExportProgressProvider>
    </ErrorBoundary>
  );
