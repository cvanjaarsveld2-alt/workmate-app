...
function TimelineItem({ event, onTap }) {
  ...
  return (
    <button
      onClick={onTap}
      className="w-full text-left flex gap-3 py-3 border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition-colors rounded-lg -mx-1 px-1"
    >
...
function ActionBtn({ icon: Icon, label, onClick, color, bg }) {
  return (
    <button
      onClick={onClick}
      className="flex flex-col items-center gap-1.5 flex-1 py-3 rounded-2xl min-h-[64px] active:scale-95 transition-transform"
      style={{ background: bg }}
    >
          ...
          <div className="flex gap-2 mt-2 flex-wrap">
            {contact.phone && (
              <>
                <a
                  ...
                  className="flex items-center gap-1 text-xs font-bold px-2.5 py-1.5 rounded-lg bg-blue-50 text-blue-700 min-h-[32px]"
                >
                  <Phone size={11} /> Call
                </a>
                <a
                  ...
                  className="flex items-center gap-1 text-xs font-bold px-2.5 py-1.5 rounded-lg bg-green-50 text-green-700 min-h-[32px]"
                >
            ...
            {contact.email && (
              <a
                ...
                className="flex items-center gap-1 text-xs font-bold px-2.5 py-1.5 rounded-lg bg-purple-50 text-purple-700 min-h-[32px]"
              >
                <Mail size={11} /> Email
              </a>
            )}
...
export function Client360Screen({
  ...
  const quotes = (data.quotes || []).filter(
    q =>
      ...
      (q.client_name && clientCo && q.client_name.toLowerCase() === clientCo.toLowerCase()),
  );
  const leads = (data.leads || []).filter(
    l =>
      l.client_id === clientId ||
      (l.client_name && clientCo && l.client_name.toLowerCase() === clientCo.toLowerCase()),
  );
  const equipment = (data.equipment || []).filter(
    e =>
      e.client_id === clientId || (e.client && clientCo && e.client.toLowerCase() === clientCo.toLowerCase()),
  );
  const activities = (data.activities || []).filter(a => a.client_id === clientId);
  const expenses = (data.expenses || []).filter(
    e =>
      e.client_id === clientId ||
      (e.client_name && clientCo && e.client_name.toLowerCase() === clientCo.toLowerCase()),
  );
  ...
  const timeline = useMemo(() => {
    ...
    followups.forEach(f =>
      events.push({
        ...
        badge: f.completed
          ? { text: "Done", bg: "#DCFCE7", color: "#166534" }
          : f.date < today
            ? { text: "Overdue", bg: "#FEF2F2", color: "#DC2626" }
            : null,
        screen: "Followups",
      }),
    );
    ...
    quotes.forEach(q => {
      ...
      events.push({
        type: "quote",
        date: q.created_at || q.sent_date,
        title: `${q.description || "Quote"} — ${formatCurrency(q.value)}`,
        subtitle: q.status,
    ...
    equipment.forEach(e =>
      events.push({
        type: "equipment",
        date: e.created_at,
        title: `${e.name} ${e.make ? `(${e.make})` : ""}`,
        subtitle: e.serial ? `S/N: ${e.serial}` : e.location || null,
        badge:
          e.service_due && daysDiff(e.service_due) !== null && daysDiff(e.service_due) < 0
            ? { text: "Service overdue", bg: "#FEF2F2", color: "#DC2626" }
            : null,
        screen: "Equipment",
      }),
    );
  ...
  if (overdueFU.length > 0)
    healthIssues.push(`${overdueFU.length} overdue follow-up${overdueFU.length !== 1 ? "s" : ""}`);
  if (unresolvedNotes > 0 && notes.some(n => n.urgency === "Critical" && !n.resolved))
    healthIssues.push("Critical notes unresolved");
  ...
  function renderTab() {
    switch (activeTab) {
      case "calls":
        return (
          <div className="px-1">
            <InteractionLog client={client} setData={setData} userId={userId} teamId={teamId} />
          </div>
        );
            ...
            {timeline.slice(0, 50).map((e, i) => (
              <TimelineItem
                ...
                onTap={() => {
                  if (e.type === "note") {
                    const n = notes.find(
                      x => x.created_at === e.date || x.note?.startsWith(e.title?.slice(0, 30)),
                    );
                    if (n) setDetailItem({ type: "note", data: n });
                    return;
                  }
      ...
      case "contacts":
        return contacts.length === 0 ? (
          <EmptyTab
            ...
            onAction={() => {
              setAddData({ name: "", phone: "", email: "", title: "" });
              setAddForm("contact");
            }}
          />
        ) : (
      ...
      case "followups":
        return followups.length === 0 ? (
          <EmptyTab
            ...
            onAction={() => {
              setAddData({ date: todayISO(), time: "", notes: "", reminder: "30_min" });
              setAddForm("followup");
            }}
          />
        ) : (
          <div className="space-y-2">
            {followups
              ...
              .map(f => (
                <button
                  key={f.id}
                  onClick={() => setDetailItem({ type: "followup", data: f })}
                  className="w-full text-left"
                >
                  <Card
                    className={`p-3 ${!f.completed && f.date < today ? "border-l-4 border-l-red-400" : ""}`}
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p
                          className={`text-sm font-bold truncate ${f.completed ? "line-through text-slate-400" : "text-slate-900"}`}
                        >
                          {f.title}
                        </p>
      ...
      case "notes":
        return notes.length === 0 ? (
          <EmptyTab
            ...
            onAction={() => {
              setAddData({ note: "", urgency: "Normal" });
              setAddForm("note");
            }}
          />
        ) : (
            ...
            {notes
              ...
              .map(n => {
                ...
                return (
                  <button
                    key={n.id}
                    onClick={() => setDetailItem({ type: "note", data: n })}
                    className="w-full text-left"
                  >
                            ...
                            <div className="flex gap-1.5 mt-2">
                              {n.media
                                ...
                                .map((m, i) => (
                                  <div
                                    key={i}
                                    className="w-10 h-10 rounded-lg bg-slate-100 overflow-hidden shrink-0"
                                  >
                                    <img
                                      src={m.url}
                                      alt=""
      ...
      case "quotes":
        return quotes.length === 0 ? (
          <EmptyTab
            ...
            onAction={() => {
              setAddData({ description: "", value: "", status: "Pending" });
              setAddForm("quote");
            }}
          />
        ) : (
            ...
            {quotes
              ...
              .map(q => {
                ...
                return (
                  <button
                    key={q.id}
                    onClick={() => setDetailItem({ type: "quote", data: q })}
                    className="w-full text-left"
                  >
                    <Card className="p-3.5">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-lg font-black mt-0.5" style={{ color: BRAND.primary }}>
                            {formatCurrency(q.value)}
                          </p>
                          <p className="text-xs text-slate-400 mt-0.5">
                            {smartDate(q.sent_date || q.created_at)}
                          </p>
                        </div>
                        <span
                          className="text-xs font-bold rounded-full px-2.5 py-1 shrink-0"
                          style={{ background: c.bg || "#F1F5F9", color: c.text || "#64748B" }}
                        >
                          {q.status}
                        </span>
                      </div>
      ...
      case "leads":
        return leads.length === 0 ? (
          <EmptyTab
            ...
            onAction={() => {
              setAddData({ title: "", stage: "New", notes: "" });
              setAddForm("lead");
            }}
          />
        ) : (
          <div className="space-y-2">
            {leads.map(l => (
              <button
                key={l.id}
                onClick={() => setDetailItem({ type: "lead", data: l })}
                className="w-full text-left"
              >
                <Card className="p-3.5">
                  <p className="text-sm font-bold text-slate-900">{l.title || "Lead"}</p>
                  {l.estimated_value && (
                    <p className="text-sm font-black mt-0.5" style={{ color: BRAND.primary }}>
                  ...
                  <div className="flex items-center gap-2 mt-1.5">
                    ...
                    <span className="text-xs text-slate-400">{smartDate(l.created_at)}</span>
                  </div>
                </Card>
              </button>
            ))}
      ...
      case "equipment":
        return equipment.length === 0 ? (
          <EmptyTab
            ...
            onAction={() => {
              setAddData({ name: "", type: "", serial: "" });
              setAddForm("equipment");
            }}
          />
        ) : (
          <div className="space-y-2">
            {equipment.map(e => (
              <button
                key={e.id}
                onClick={() => setDetailItem({ type: "equipment", data: e })}
                className="w-full text-left"
              >
                <Card className="p-3.5">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      {e.serial && <p className="text-xs text-slate-400 mt-0.5">S/N: {e.serial}</p>}
                    </div>
      ...
      case "expenses":
        return expenses.length === 0 ? (
          ...
          <div className="space-y-2">
            ...
            {expenses
              .sort((a, b) =>
                (b.expense_date || b.created_at || "").localeCompare(a.expense_date || a.created_at || ""),
              )
              .slice(0, 20)
              .map(e => (
                <button
                  key={e.id}
                  onClick={() => setDetailItem({ type: "expense", data: e })}
                  className="w-full text-left"
                >
                  <Card className="p-3.5">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-500">
                          {e.category || ""} · {smartDate(e.expense_date || e.created_at)}
                        </p>
                      </div>
                      <p className="text-sm font-black shrink-0" style={{ color: BRAND.primary }}>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-start gap-3">
        <button
          onClick={onBack}
          className="p-2.5 rounded-xl border-2 border-slate-200 bg-white text-slate-500 min-w-[44px] min-h-[44px] flex items-center justify-center shrink-0 mt-0.5"
        >
          <ArrowLeft size={18} />
        </button>
        <div className="flex-1 min-w-0">
        ...
        {client.phone && (
          <>
            <ActionBtn
              icon={Phone}
              label="Call"
              onClick={() => window.open(telLink(client.phone))}
              color="#1E40AF"
              bg="#DBEAFE"
            />
            <ActionBtn
              icon={ExternalLink}
              label="WhatsApp"
              onClick={() => window.open(whatsappLink(client.phone, `Hi ${client.contact || ""}`))}
              color="#166534"
              bg="#DCFCE7"
            />
          </>
      ...
      <div>
        ...
        {(totalExpenses > 0 || equipment.length > 0 || openLeads.length > 0) && (
          <button
            ...
            className="w-full flex items-center justify-center gap-1 pt-2 text-xs font-bold text-slate-400"
          >
            {showAllStats ? "Less" : "More stats"}{" "}
            {showAllStats ? <ChevronUp size={12} /> : <ChevronDown size={12} />}
          </button>
        )}
        <AnimatePresence>
          {showAllStats && (
            ...
            >
              ...
              {lastContact && (
                <p className="text-xs text-slate-400 text-center mt-2">
                  Last contact: {smartDate(lastContact)} ({daysSinceLast}d ago)
                </p>
              )}
            </motion.div>
          )}
                ...
                {addForm === "followup" && (
                  <>
                    <Field
                      label="Title"
                      value={addData.title || ""}
                      onChange={v => setAddData(d => ({ ...d, title: v }))}
                      placeholder="What needs to happen?"
                    />
                    <Field
                      label="Date"
                      type="date"
                      value={addData.date || todayISO()}
                      onChange={v => setAddData(d => ({ ...d, date: v }))}
                    />
                    <Field
                      label="Time (optional)"
                      type="time"
                      value={addData.time || ""}
                      onChange={v => setAddData(d => ({ ...d, time: v }))}
                    />
                    <Field
                      label="Notes (optional)"
                      value={addData.notes || ""}
                      onChange={v => setAddData(d => ({ ...d, notes: v }))}
                      multiline
                    ...
                    <button
                      ...
                      className="w-full py-4 rounded-2xl text-white font-black text-sm min-h-[52px]"
                      style={{ background: BRAND.primary }}
                    >
                      Save Follow-up
                    </button>
                ...
                {addForm === "note" && (
                  <>
                    <Field
                      label="Note"
                      value={addData.note || ""}
                      onChange={v => setAddData(d => ({ ...d, note: v }))}
                      multiline
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        {["Normal", "Urgent", "Critical"].map(u => (
                          <button
                            key={u}
                            onClick={() => setAddData(d => ({ ...d, urgency: u }))}
                            className={
                              "flex-1 py-2 rounded-xl text-xs font-bold transition-all " +
                              (addData.urgency === u ? "text-white" : "bg-slate-100 text-slate-500")
                            }
                            style={
                              addData.urgency === u
                                ? {
                    ...
                    <Field
                      label="Resolve by (optional)"
                      type="date"
                      value={addData.resolve_by || ""}
                      onChange={v => setAddData(d => ({ ...d, resolve_by: v }))}
                    />
                    <button
                      onClick={() => {
                        ...
                        const item = withTeamId(
                          {
                            ...
                            client_id: clientId || null,
                            note: addData.note,
                            urgency: addData.urgency || "Normal",
                            resolve_by: addData.resolve_by || null,
                            resolved: false,
                            media: [],
                            sync_status: "pending",
                            created_at: new Date().toISOString(),
                ...
                {addForm === "contact" && (
                  <>
                    <Field
                      label="Name"
                      value={addData.name || ""}
                      onChange={v => setAddData(d => ({ ...d, name: v }))}
                      placeholder="Full name"
                    />
                    <Field
                      label="Title / Role"
                      value={addData.ctitle || ""}
                      onChange={v => setAddData(d => ({ ...d, ctitle: v }))}
                    ...
                    <Field
                      ...
                      type="tel"
                      value={addData.phone || ""}
                      onChange={v => setAddData(d => ({ ...d, phone: v }))}
                    ...
                    <Field
                      ...
                      type="email"
                      value={addData.email || ""}
                      onChange={v => setAddData(d => ({ ...d, email: v }))}
                        ...
                        const item = withTeamId(
                          {
                            ...
                            user_id: userId,
                            name: addData.name,
                            title: addData.ctitle || "",
                            phone: addData.phone || "",
                            email: addData.email || "",
                            company: client.company,
                            client_id: clientId || null,
                            status: "Lead",
                            met_at: client.company,
                            met_date: todayISO(),
                            sync_status: "pending",
                            created_at: new Date().toISOString(),
                          },
                ...
                {addForm === "lead" && (
                  <>
                    <Field
                      ...
                      value={addData.title || ""}
                      onChange={v => setAddData(d => ({ ...d, title: v }))}
                      placeholder="e.g. Tyre Handler opportunity"
                    />
                    <Field
                      label="Estimated value (R)"
                      type="number"
                      value={addData.estimated_value || ""}
                      onChange={v => setAddData(d => ({ ...d, estimated_value: v }))}
                      placeholder="0"
                    />
                    <Field
                      label="Notes"
                        ...
                        const item = withTeamId(
                          {
                            ...
                            client_id: clientId || null,
                            estimated_value: parseFloat(addData.estimated_value) || 0,
                            notes: addData.notes || "",
                            stage: "New",
                            captured_by: userEmail?.split("@")[0] || "",
                            sync_status: "pending",
                            created_at: new Date().toISOString(),
                          },
                          teamId,
                ...
                {addForm === "quote" && (
                  <>
                    <Field
                      ...
                      onChange={v => setAddData(d => ({ ...d, description: v }))}
                      multiline
                    ...
                    <Field
                      ...
                      type="number"
                      value={addData.value || ""}
                      onChange={v => setAddData(d => ({ ...d, value: v }))}
                      placeholder="0.00"
                    />
                    <button
                      onClick={() => {
                        ...
                        const item = withTeamId(
                          {
                            ...
                            value: parseFloat(addData.value) || 0,
                            status: "Pending",
                            sent_date: todayISO(),
                            sync_status: "pending",
                            created_at: new Date().toISOString(),
                ...
                {addForm === "equipment" && (
                  <>
                    <Field
                      ...
                      value={addData.name || ""}
                      onChange={v => setAddData(d => ({ ...d, name: v }))}
                      placeholder="e.g. Tyre Handler TH-500"
                    />
                    <Field
                      label="Type"
                      value={addData.type || ""}
                      onChange={v => setAddData(d => ({ ...d, type: v }))}
                      placeholder="e.g. Tyre Handler"
                    />
                    <Field
                      label="Serial number"
                      value={addData.serial || ""}
                      onChange={v => setAddData(d => ({ ...d, serial: v }))}
                      placeholder="Serial / asset no."
                    />
                    <Field
                      label="Service due (optional)"
                      type="date"
                      value={addData.service_due || ""}
                      onChange={v => setAddData(d => ({ ...d, service_due: v }))}
                    />
                    <button
                      onClick={() => {
                        if (!addData.name?.trim()) return;
                        const item = withTeamId(
                          {
                            ...
                            client: client.company,
                            client_id: clientId || null,
                            location: client.branch || client.company,
                            service_due: addData.service_due || null,
                            notes: "",
                            media: [],
                            sync_status: "pending",
                            created_at: new Date().toISOString(),
            ...
            >
              ...
              <div className="overflow-y-auto px-5 pb-8 space-y-4" style={{ maxHeight: "calc(82vh - 24px)" }}>
                ...
                <div className="flex items-start justify-between gap-3 pt-1">
                  ...
                  <button
                    onClick={() => setDetailItem(null)}
                    className="w-9 h-9 rounded-xl flex items-center justify-center bg-slate-100 text-slate-500 shrink-0"
                  >
                    <X size={18} />
                  </button>
                </div>
                    ...
                    return (
                      <>
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {n.resolve_by && (
                            <span className="text-xs text-slate-500">Due: {smartDate(n.resolve_by)}</span>
                          )}
                        </div>
                ...
                {/* FOLLOW-UP */}
                {detailItem.type === "followup" &&
                  (() => {
                    const f = detailItem.data;
                    return (
                      <>
                        <div className="flex items-center gap-2 flex-wrap">
                          <span
                            className={`text-xs font-bold px-2.5 py-1 rounded-full ${f.completed ? "bg-green-100 text-green-700" : f.date < today ? "bg-red-100 text-red-700" : "bg-blue-50 text-blue-700"}`}
                          >
                        ...
                        {f.reminder && f.reminder !== "none" && (
                          <p className="text-sm text-slate-500">Reminder: {f.reminder.replace(/_/g, " ")}</p>
                        )}
                      </>
                    );
                  })()}
                  ...
                  (() => {
                    ...
                    return (
                      <>
                        ...
                        {q.sent_date && (
                          <p className="text-sm text-slate-500">Sent: {smartDate(q.sent_date)}</p>
                        )}
                        {q.description && (
                          <div className="rounded-xl bg-slate-50 p-4">
                    ...
                    return (
                      <>
                        <div className="rounded-xl bg-slate-50 p-4 space-y-1.5">
                          ...
                          {e.make && (
                            <p className="text-sm text-slate-700">
                              <span className="font-bold">Make:</span> {e.make}
                            </p>
                          )}
                          {e.model && (
                            <p className="text-sm text-slate-700">
                              <span className="font-bold">Model:</span> {e.model}
                            </p>
                          )}
                          {e.serial && (
                            <p className="text-sm text-slate-700">
                              <span className="font-bold">Serial:</span> {e.serial}
                            </p>
                          )}
                          {e.location && (
                            <p className="text-sm text-slate-700">
                          ...
                          {e.service_due && (
                            <p
                              ...
                              style={{ color: daysDiff(e.service_due) < 0 ? "#DC2626" : "#166534" }}
                            >
                              Service due: {smartDate(e.service_due)}
                            </p>
                          )}
                        </div>
                        {e.notes && (
                          <div className="rounded-xl bg-slate-50 p-4">
                            <p className="text-sm text-slate-800">{e.notes}</p>
                          </div>
                        )}
                ...
                {/* LEAD */}
                {detailItem.type === "lead" &&
                  (() => {
                    const l = detailItem.data;
                    return (
                      <>
                        <div className="flex items-center gap-2">
                          <span
                        ...
                        {l.follow_up_date && (
                          <p className="text-sm text-slate-500">Follow up: {smartDate(l.follow_up_date)}</p>
                        )}
                      </>
                    );
                  })()}
                    ...
                    return (
                      <>
                        <div className="rounded-xl bg-slate-50 p-4 space-y-1.5">
                          ...
                          {e.currency !== "ZAR" && (
                            <p className="text-xs text-slate-400">
                              {e.currency} {e.amount} @ {e.exchange_rate}
                            </p>
                          )}
                          {e.category && (
                            <p className="text-sm text-slate-700">
                          ...
                          {e.payment_method && (
                            <p className="text-sm text-slate-700">
                              <span className="font-bold">Payment:</span> {e.payment_method}
                            </p>
                          )}
                          {e.expense_date && (
                            <p className="text-sm text-slate-700">
                              <span className="font-bold">Date:</span> {smartDate(e.expense_date)}
                            </p>
                          )}
                        </div>
                        {e.notes && (
