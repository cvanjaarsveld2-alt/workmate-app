...
export function DiagnosticsScreen({ data, setData, userId, isOnline, onBack, onBackfill }) {
  ...
  async function copyReport() {
    const lines = [
      ...
      ...Object.entries(funcStatus).map(
        ([n, s]) =>
          `  · ${n}: ${s.state}${s.ms ? ` (${s.ms}ms)` : ""}${s.err ? ` — ${s.err}` : ""}${s.code ? ` — HTTP ${s.code}` : ""}`,
      ),
      "",
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        <button
          onClick={onBack}
          className="p-2 rounded-xl text-slate-600 hover:bg-slate-100 min-w-[40px] min-h-[40px] flex items-center justify-center"
        >
          <ArrowLeft size={20} />
        </button>
        <div>
      ...
      <Card className="p-4">
        ...
        <Row
          ...
          hint={
            pending.length > 50
              ? "Large queue — something may be stuck."
              : pending.some(i => (i.attempts || 0) >= 2)
                ? `${pending.filter(i => (i.attempts || 0) >= 2).length} item(s) have failed and are retrying — will move to failed after 5 attempts.`
                : null
          }
        />
        <Row
      ...
      <Card className="p-4">
        ...
        <Row
          ...
          status="ok"
          hint="Scheduled field work available to the app."
        />
        <Row
          icon={CreditCard}
          label="Invoices"
          ...
          hint={
            invoices.length
              ? `${money(invoiceOutstanding)} outstanding · ${money(invoicePaid)} paid`
              : "No invoices loaded."
          }
        />
        <Row
          ...
          status="ok"
          hint="Recorded payments are reconciled against invoices by the database."
        />
      </Card>

      {(() => {
        const missingZAR = (data.expenses || []).filter(
          e => e.currency && e.currency !== "ZAR" && (!e.amount_zar || e.amount_zar <= 0),
        );
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    {missingZAR.length} foreign-currency expense{missingZAR.length !== 1 ? "s" : ""} missing a
                    ZAR amount
                  </p>
                </div>
              </div>
      ...
      {failed.length > 0 && (
        <Card className="p-4">
          ...
          <p className="text-xs text-slate-400 mt-3 leading-relaxed">
            Most often this means a column is missing on the table or an RLS rule is blocking the write. Open
      ...
      <Card className="p-4">
        ...
        <p className="text-xs text-slate-400 mt-3 leading-relaxed">
          Tap “Check now” to ping each function. Green = reachable. Red = not deployed, crashed, or
          unauthorized.
        </p>
      </Card>
      ...
      <Card className="p-4">
        ...
        {crashes.length === 0 ? (
          ...
          <div className="space-y-2">
            ...
            {crashes.length > 5 && <p className="text-xs text-slate-400">…and {crashes.length - 5} more</p>}
          </div>
        )}
      </Card>
      ...
      <Card className="p-4">
        ...
        <p className="text-xs text-slate-400 mt-2 leading-relaxed">
          Bundles everything on this page into a text block on your clipboard — paste into a message instead
