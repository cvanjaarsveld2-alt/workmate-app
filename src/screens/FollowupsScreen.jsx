...
function FollowupCard({ f, today, onToggle, onEdit, onDelete, onSendInfo, onShare, showShare }) {
  ...
  const displayText = isLong && !expanded ? f.title.slice(0, LIMIT).trimEnd() + "…" : f.title;
    ...
    <Card className={`p-3.5 ${isOverdue ? "border-l-4 border-l-red-400" : ""}`}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex flex-col gap-2 shrink-0">
          ...
          <button
            onClick={onDelete}
            className="p-2.5 rounded-xl bg-slate-50 text-slate-400 hover:text-red-600 transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center"
          >
            <Trash2 size={14} />
          </button>
        </div>
...
export function FollowupsScreen({
  ...
  function startEdit(f) {
    setForm({
      ...
      time: f.time || "09:00",
      reminder: f.reminder || "morning",
      notes: f.notes || "",
      linked_note_id: f.linked_note_id || "",
    ...
    return (
      <Card className="p-4 stack-y-3">
        ...
        <Field
          ...
          placeholder="e.g. Call mine buyer re quote"
          required
        />
        <ClientSelector
          label="Client"
          value={form.client_id}
          onChange={v => setForm(f => ({ ...f, client_id: v, linked_note_id: "" }))}
          clients={clients}
        />
        ...
        <div className="flex gap-2">
          <Btn className="flex-1" onClick={saveFollowup}>
            <Save size={15} />
            {isEdit ? "Update" : "Add Follow-up"}
          </Btn>
    ...
    return (
      <FollowupCard
        ...
        onSendInfo={() => {
          ...
          setSendInfo({
            name: f.client || client?.contact || "",
            email: client?.email || "",
            phone: f.clientPhone || client?.phone || "",
          });
        }}
      />
    );
  ...
  return (
    <div className="stack-y-4">
      ...
      <ShareSheet
        ...
        onAssign={(uid, email) => {
          ...
          const updated = {
            ...fu,
            assigned_to_user_id: uid,
            assigned_to: email?.split("@")[0] || email,
            sync_status: "pending",
          };
          setData(d => ({
            ...d,
                  ...
                  {[
                    ...
                    <button
                      ...
                      onClick={async () => {
                        if (!opt.days) {
                          setForm({
                            title: nextActionPrompt.title,
                            client_id: nextActionPrompt.client_id || null,
                            client: nextActionPrompt.client || "",
                            branch: nextActionPrompt.branch || "",
                            date: todayISO(),
                            time: "",
                            reminder: "30_min",
                            notes: "",
                        ...
                        const nd = new Date();
                        nd.setDate(nd.getDate() + opt.days);
      ...
      <div className="flex items-center justify-between">
        <PageHeader
          title="Follow-ups"
          subtitle={`${pendingTotal} pending${overdueCount > 0 ? ` · ${overdueCount} overdue` : ""}`}
        />
        <Btn
          size="sm"
          onClick={() => {
        ...
        >
          {showForm || editId ? <X size={15} /> : <Plus size={15} />}
      ...
      {filtered.length === 0 && (
        <Empty
          title={filter === "Done" ? "No completed follow-ups" : "All clear!"}
          text="No follow-ups in this category."
          icon={Calendar}
        />
      )}
        ...
        <div className="stack-y-4">
          {bucketOrder
            ...
            .map(bucket => (
              <div key={bucket}>
                <p
                  className={`text-sm font-bold uppercase tracking-wider px-1 mb-2 ${bucket === "Overdue" ? "text-red-600" : bucket === "Today" ? "text-blue-600" : "text-slate-400"}`}
                >
