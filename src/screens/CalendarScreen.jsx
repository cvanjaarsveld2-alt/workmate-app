...
function readCalendarNotes(item) {
  ...
  lines.forEach(line => {
    ...
    if (line.startsWith("Related to: ")) related_to = line.slice(12);
      ...
      return; // rebuilt from contact_id on save
...
function itemLocation(item, clients) {
  return (
    item.location ||
    readCalendarNotes(item).location ||
    clients.find(client => client.id === item.client_id)?.location ||
    ""
  );
}
...
export function CalendarScreen({ data, setData, userId, teamId, onNavigate }) {
  ...
  const clients = (data.clients || []).filter(
    client => !client.user_id || client.user_id === userId || client.assigned_to_user_id === userId,
  );
  const calendarItems = useMemo(() => {
    ...
    (data.notes || [])
      .filter(
        note =>
          (note.user_id === userId || note.assigned_to_user_id === userId) &&
          note.resolve_by &&
          !note.resolved,
      )
      .forEach(note => {
        items.push({
          id: note.id,
          date: note.resolve_by,
          time: "",
          title: (note.note || "Note").slice(0, 100),
          client: note.client || "",
          completed: !!note.resolved,
          _source: "note",
          _kind: "Note",
          _screen: "Notes",
        });
    ...
    (data.equipment || [])
      .filter(item => (item.user_id === userId || item.assigned_to_user_id === userId) && item.service_due)
      .forEach(item => {
        const client = clients.find(clientItem => clientItem.id === item.client_id);
        items.push({
          id: item.id,
          date: item.service_due,
          time: "",
          title: `Service: ${item.name || "Equipment"}`,
          client: item.client || client?.company || "",
          completed: false,
    ...
    (data.quotes || [])
      .filter(quote => (quote.user_id === userId || quote.assigned_to_user_id === userId) && quote.sent_date)
      .forEach(quote => {
        items.push({
          id: quote.id,
          date: quote.sent_date,
          time: "",
          title: `Quote: ${quote.description || quote.client_name || "Quote"}`,
  ...
  const locations = [
    ...new Set(
      dayItems
        ...
        .map(item => itemLocation(item, clients))
        .filter(Boolean),
    ),
  ];
    ...
    return (
      <Card key={item.id} className={`overflow-hidden ${item.completed ? "opacity-60" : ""}`}>
        <button
          type="button"
          onClick={openItem}
          className="w-full text-left p-3.5 flex gap-3 active:bg-slate-50"
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            {!compact && parsed.related_to && (
              <p className="text-xs text-slate-400 mt-1">Related to: {parsed.related_to}</p>
            )}
          </div>
        ...
        {!compact && editable && (
          <div className="border-t border-slate-100 flex divide-x divide-slate-100">
            ...
            <button
              ...
              className="flex-1 min-h-[44px] text-xs font-bold text-blue-600 flex items-center justify-center gap-1.5 active:bg-blue-50"
            >
              <Pencil size={13} /> Edit
            </button>
            <button
              onClick={() => removeItem(item)}
              className="w-14 min-h-[44px] text-red-500 flex items-center justify-center active:bg-red-50"
              aria-label="Delete"
            >
              <Trash2 size={14} />
            </button>
  ...
  return (
    <div className="space-y-4 pb-24">
      <div className="flex items-end justify-between px-1">
        ...
        <button
          onClick={openNew}
          className="w-12 h-12 rounded-full text-white shadow-lg flex items-center justify-center active:scale-95 transition-transform"
      ...
      <Card className="overflow-hidden">
        <div className="px-3 pt-3 flex items-center justify-between">
          <button
            onClick={() => moveMonth(-1)}
            className="w-11 h-11 rounded-full flex items-center justify-center text-slate-600 active:bg-slate-100"
        ...
        {displayMode === "month" ? (
          ...
          >
            ...
            <div className="grid grid-cols-7 px-1 sm:px-2 pb-2">
              {cells.map(cell => {
                const items = [...(byDate[cell.date] || [])].sort((a, b) =>
                ...
                return (
                  <button
                    ...
                    className={`relative min-h-[58px] sm:min-h-[76px] py-1 rounded-xl flex flex-col items-center active:bg-slate-100 ${!cell.currentMonth ? "opacity-30" : ""}`}
                  >
                    <span
                      className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${selected ? "text-white" : isToday ? "ring-2" : "text-slate-800"}`}
                      style={
                        selected
                          ? { background: BRAND.primary }
                          : isToday
                            ? { color: BRAND.primary, ringColor: BRAND.primary }
                            : {}
                      }
                    >
                      {cell.day}
          ...
          <div className="border-t border-slate-100 p-3 max-h-[520px] overflow-y-auto">
            {Object.keys(scheduleGroups).length === 0 ? (
              <div className="py-12 text-center">
                <CalendarIcon size={28} className="mx-auto text-slate-200" />
                <p className="text-sm font-bold text-slate-400 mt-2">No events this month</p>
              </div>
            ) : (
      ...
      {displayMode === "month" && (
        <>
          {locations.length > 0 && (
            <button
              onClick={() => window.open(routeUrl(locations), "_blank", "noopener,noreferrer")}
              className="w-full py-3 rounded-2xl bg-green-50 border border-green-200 text-green-700 text-sm font-black flex items-center justify-center gap-2"
            >
              <Navigation size={16} /> Plan route · {locations.length} stop{locations.length !== 1 ? "s" : ""}
            </button>
          )}
          <div className="flex items-center justify-between px-1">
            ...
            <button
              onClick={openNew}
              className="text-sm font-bold flex items-center gap-1 min-h-[44px] px-2"
          ...
          {!dayItems.length ? (
            <Card className="p-8 text-center">
              ...
              <button onClick={openNew} className="mt-3 text-sm font-bold" style={{ color: BRAND.primary }}>
                Add an event
              </button>
            </Card>
          ) : (
          ...
          <div className="bg-[#F7F7FA] w-full sm:max-w-xl sm:rounded-3xl rounded-t-3xl max-h-[92vh] overflow-y-auto shadow-2xl">
            <div className="sticky top-0 z-10 bg-white/95 backdrop-blur border-b border-slate-200 px-4 py-3 flex items-center justify-between">
              ...
              <button
                ...
                className="min-h-[44px] px-1 text-sm font-black"
                style={{ color: BRAND.primary }}
              >
                Done
              </button>
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="bg-white rounded-2xl overflow-hidden border border-slate-200 divide-y divide-slate-100">
                <label className="min-h-[56px] px-4 flex items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    onClick={() => setForm(current => ({ ...current, allDay: !current.allDay }))}
              ...
              <div className="bg-white rounded-2xl overflow-hidden border border-slate-200 divide-y divide-slate-100">
                <div className="px-4 py-3">
                  ...
                  <ClientSelector
                    ...
                    clients={clients}
                    placeholder="No client — tap to search…"
                ...
                <label className="min-h-[56px] px-4 flex items-center gap-3">
                  <Bell size={17} className="text-slate-400" />
                  <span className="font-semibold text-slate-800 shrink-0">Alert</span>
                  <select
                    value={form.reminder}
                ...
                <label className="min-h-[56px] px-4 flex items-center gap-3">
                  <Navigation size={17} className="text-slate-400" />
                  <input
              ...
              <button
                onClick={saveCalendarItem}
                className="w-full min-h-[54px] rounded-2xl text-white font-black flex items-center justify-center gap-2"
                style={{ background: BRAND.primary }}
              >
                <Save size={17} /> {editId ? "Save Changes" : "Add Event"}
              </button>
              {editId && (
                <button
                  onClick={() => removeItem(followups.find(item => item.id === editId))}
                  className="w-full min-h-[54px] rounded-2xl bg-white border border-red-200 text-red-600 font-black flex items-center justify-center gap-2"
                >
                  <Trash2 size={17} /> Delete Event
                </button>
              )}
            </div>
          </div>
