...
const CONFIG = {
  ...
  leads: {
    title: r => r.title || r.client_name || "Opportunity",
    icon: Building2,
    fields: [
      ["Client", "client_name"],
...
function displayValue(key, value) {
  ...
  if (key.includes("date") && /^\d{4}-\d{2}-\d{2}/.test(String(value)))
    return smartDate(String(value).slice(0, 10));
  ...
  if (!config || !record)
    return (
      <div className="stack-y-4">
        ...
        <Card className="p-8 text-center text-slate-400">Record not found</Card>
      </div>
    );
    ...
    <div className="stack-y-4">
      ...
      <Card className="p-5">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h2 className="text-lg font-black text-slate-900">{config.title(record)}</h2>
            <p className="text-xs text-slate-400">Owner: {owner?.email || record.user_id || "Unknown"}</p>
          </div>
        </div>
